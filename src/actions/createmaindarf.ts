...
//    *   // 🟢 async function: Server action সবসময় async হতে হবে। 
  ...
  const modifiedData = {
    ...
    thumbnail: imageURL,
    isFeatured: Boolean(blogInfo.isFeatured), // boolean এ cast করা
    tags: blogInfo.tags
      .toString()
      .split(",")
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/blogs/`, {
    ...
    headers: {
      ...
      Authorization:   "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOjEsInJvbGUiOiJvd25lciIsImVtYWlsIjoidGlwdXNhaGlsLmN0Z0BnbWFpbC5jb20iLCJpYXQiOjE3NTkxNjU2MTcsImV4cCI6MTc2MDAyOTYxN30.IXBu2ab4VLpZNZmPQuV6ekMf3eNs5GJD2BDgR3taQJM",
   ...
   * - next:{ tags } দেওয়ার মানে হচ্ছে এই fetch call caching এর সাথে BLOG ট্যাগ bind হচ্ছে।
  ...
  if (result?.success === true) {
     ...
     * - revalidateTag("BLOG") দিলে cached GET requests (যেগুলোতে next:{tags:["BLOG"]} ছিলো)
     *   automatically invalidate হয়ে fresh data নিয়ে আসবে।
     * - mutation করার পরে সবসময় explicitly revalidate করা best practice।
    //  *     // 🟢 BLOG cache invalidate করা হলো।
    // 👉 যাতে নতুন blog create হবার পর সাথে সাথে latest data দেখা যায়। 
     */
    // revalidatePath("/blogs"); // ❌ এক্ষেত্রে ট্যাগ based invalidation বেশি scalable (industry standard)।
...
❌ Common Mistakes:
- শুধুমাত্র fetch এর মধ্যে next:{tags:["BLOG"]} দিয়ে cache invalidate আশা করা (এটা হয় না)।
...
// Example: <form action={createBlog}>...</form>
